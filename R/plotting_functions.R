...
plot_component <- function(input_list = NULL,
    ...
    if (!is.null(geneinfo_df)) {
        component_plots[[1]] <- plot_comp_chr(input_list,
                                              comp_idx = comp_idx,
                                              geneinfo_df = geneinfo_df)
    } else {
        component_plots[[1]] <- plot_comp_no_geneinfo(input_list,
                                                      comp_idx = comp_idx)
    }
    ...
    if (is.null(coeff_plot_df$covar) &
        ...
        component_plots[[3]] <- ggplot(coeff_plot_df,
                                       aes(x = sample_idx, y = coeff)) +
            geom_point() + theme_bw() + theme(legend.position = "none")

        component_plots[[2]] <-
            ggplot(coeff_plot_df, aes(x = coeff)) +
        ...
        component_plots[[2]] <-
            ...
            theme(legend.position = "none")
               ...
               !is.null(coeff_plot_df$mclust)) {
        component_plots[[3]] <- ggplot(coeff_plot_df, aes(x = mclust,
                                                          y = coeff,
                                                          col = mclust)) +
...
plot_summary <- function(input_list = NULL,
    ...
    if (class(input_list) == "PCAobject") {
        ...
        summary_plots[[2]] <-
            ggplot(proj_df, aes(x = comp1, y = comp2)) +
            geom_point() + theme_bw()
        if (attr(input_list, "covar_cor") == "yes") {
