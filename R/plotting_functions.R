...
#' @param plot_here If set to TRUE it will generate the plots in the current session, and if set to FALSE it will return a list of plots only
#'                  and not generate plots in the current session.
...
#' plot_component(pca_object, comp_idx = 1)
#'
#' @export
plot_component <- function(input_list = NULL,
    ...
    if(is.null(coeff_plot_df$covar) & is.null(coeff_plot_df$mclust)){
        ...
        if(class(coeff_plot_df$covar) == "numeric"){
            ...
            component_plots[[3]] <- ggplot(coeff_plot_df, aes(x = covar, y = coeff, col = covar)) +
                geom_jitter(width = 0.3) + theme_bw() + xlab(covar_name) + theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
plot_summary <- function(input_list = NULL,
    ...
    summary_df <- data.frame("var_percent" = input_list$percent_var[var_order],
    ...
    if(class(input_list) == "PCAobject"){
        ...
        summary_plots[[1]] <- summary_plots[[1]] + xlab("IC ordered by variance")
